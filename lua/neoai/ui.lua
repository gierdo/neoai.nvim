...
local colorizer = require("baleia.api")
local colorizer_options = require("baleia.options")
...
local function conceal_ansi_codes(bufnr)
  -- Use the provided buffer number or default to current buffer
  local buf = bufnr or 0

  -- Set up syntax matching for ANSI escape codes
  vim.api.nvim_buf_call(buf, function()
    vim.cmd([[
      syntax match AnsiEscapeCode '\e\[[0-9;]*m' conceal
      setlocal conceallevel=3
      setlocal concealcursor=nvic
    ]])
  end)

  -- Set up an autocmd for when yanking occurs in this buffer
  vim.api.nvim_create_autocmd("TextYankPost", {
    buffer = bufnr,
    callback = function(opts)
      -- Process the yanked text to remove concealed parts if needed
      local register = vim.v.register
      local regcontents = vim.fn.getreg(register)
      -- Strip ANSI codes from yanked text
      local cleaned = vim.fn.substitute(regcontents, "\\e\\[[0-9;]*m", "", "g")
      vim.fn.setreg(register, cleaned)
    end,
  })
end
...
M.append_to_output = function(txt, type)
  ...
  for i, line in ipairs(lines) do
    ...
    local colorizer_opts = colorizer_options.with_defaults()
    colorizer_opts.strip_ansi_codes = false
    colorizer_opts.async = false
    colorizer.once(colorizer_opts, buffer)
  end
end
